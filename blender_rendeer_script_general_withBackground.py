#model with cursor on the object & in center of the cs(0,0,0), face to y positive
import bpy
import os
import math
...
CONFIGS = {
    'light_intensities': [
        {'name': 'low_1', 'main_intensity': 1.0, 'fill_intensity': 0.3},
        {'name': 'medium_3', 'main_intensity': 3.0, 'fill_intensity': 1.0},
        {'name': 'high_6', 'main_intensity': 6.0, 'fill_intensity': 2.0}
    ],
    'light_positions': [
        {'name': 'front', 'main_pos': (3, -3, 5), 'fill_pos': (-1, -1, 3), 'back_pos': (0, 2, 3)},
        {'name': 'side', 'main_pos': (3, 0, 4), 'fill_pos': (-1, 1, 3), 'back_pos': (0, 2, 3)},
        {'name': 'back', 'main_pos': (-3, 3, 4), 'fill_pos': (1, -1, 3), 'back_pos': (0, -2, 3)}
    ],
    'distances': [
        {'name': 'close', 'radius': 2.0, 'height': 1.0},
        {'name': 'normal', 'radius': 3.0, 'height': 1.5},
        {'name': 'far', 'radius': 4.0, 'height': 2.0}
    ],
    'object_angles': [
        {'name': 'front_0', 'angle_deg': 0},
        {'name': 'front_right_30', 'angle_deg': 30},
        {'name': 'front_right_45', 'angle_deg': 45},
        {'name': 'right_90', 'angle_deg': 90},
        {'name': 'right_110', 'angle_deg': 110},
        {'name': 'back_right_135', 'angle_deg': 135},
        {'name': 'back_right_150', 'angle_deg': 150},
    ...
    for light_position in CONFIGS['light_positions']:
        ...
        for distance_config in CONFIGS['distances']:
            for object_angle in CONFIGS['object_angles']:
                ...
                filename = f"{OBJECT_NAME}_{light_intensity['name']}_{light_position['name']}_{distance_config['name']}_{object_angle['name']}_withBackground.png"
